...
- `PLUGIN` manifest with keys:
  ...
  - `always_confirm` (bool; defaults to `False`; when true, the user must approve each run)
...
- If `always_confirm` is true, the user is prompted before the plugin runs (used by bash).
