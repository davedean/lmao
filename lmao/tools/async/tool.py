...
PLUGINS = [
    ...
    },
    {
        ...
        "allow_in_yolo": True,
        "always_confirm": False,
        "input_schema": "v2 target: file path; v2 args: {start_at:'end'|'start'}",
        "usage": [
    ...
    },
    {
        ...
        "usage": [
            ...
            "{\"tool\":\"async_bash\",\"target\":\"\",\"args\":\"sleep 20\"}",
            "{\"tool\":\"async_bash\",\"target\":\"\",\"args\":{\"command\":\"sleep 20\"}}",
        ],
        "details": [
...
def _extract_async_bash_command(args: str, target: str) -> str:
...
def run(
    ...
    if tool_name == "async_tail":
        ...
        if isinstance(args, dict):
            ...
            start_at = str(kv.get("start_at") or "end").strip() or "end"
        ...
        data = {"job_id": job_id, "path": str(path), "start_at": start_at}
    ...
    if tool_name == "async_bash":
        if isinstance(args, dict):
            ...
            command = _extract_async_bash_command(str(args), target)
            cwd_hint = ""
        if not command:
